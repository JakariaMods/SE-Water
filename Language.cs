namespace Jakaria
{
    public class Language
    {
        public string EnglishName = "english";
        public string NativeName = "English";

        public string WaterModVersion = "Water Mod V{0} by Jakaria.";
        public string TranslationAuthor = "Jakaria";

        public string NoWater = "This planet does not have water";
        public string NoPlanet = "No nearby planet found.";

        public string SetLanguage = "Set the language to '{0}'.";
        public string SetLanguageNoParse = "Couldn't parse language '{0}', this language may not have been translated.";

        public string GetQuality = "Your water quality is '{0}'.";
        public string SetQuality = "Set water quality to '{0}'.";
        public string SetQualityNoParse = "Couldn't parse water quality '{0}'.";

        public string ToggleRenderCOB = "Toggled rendering of center of buoyancy.";

        public string GetBuoyancy = "The water's buoyancy multiplier is '{0}'.";
        public string SetBuoyancy = "Set the water's buoyancy multiplier to '{0}'.";
        public string SetBuoyancyNoParse = "Couldn't parse buoyancy multiplier '{0}'.";

        public string GetViscosity = "The water's viscosity is '{0}'.";
        public string SetViscosity = "Set the water's viscosity to '{0}'.";
        public string SetViscosityNoParse = "Couldn't parse viscosity '{0}'.";

        public string RetiredCommand = "This command has been retired, see the workshop page for updated info.";

        public string GetRadius = "The water's radius is '{0}'.";
        public string SetRadius = "Set the water's radius to '{0}'.";
        public string SetRadiusNoParse = "Couldn't parse radius '{0}'.";

        public string GetWaveHeight = "The water's wave height is '{0}m'.";
        public string SetWaveHeight = "Set the water's wave height to '{0}m'.";
        public string SetWaveHeightNoParse = "Couldn't parse wave height '{0}'.";

        public string Reset = "Reset the water settings on this planet.";

        public string GetCurrentSpeed = "The water's current speed is '{0}'.";
        public string SetCurrentSpeed = "Set the water's current speed to '{0}'.";
        public string SetCurrentSpeedNoParse = "Couldn't parse current speed '{0}'.";

        public string GetCurrentScale = "The water's current scale is '{0}'.";
        public string SetCurrentScale = "Set the water's current scale to '{0}'.";
        public string SetCurrentScaleNoParse = "Couldn't parse current scale '{0}'.";

        public string GetWaveSpeed = "The water's wave speed is '{0}'.";
        public string SetWaveSpeed = "Set the water's wave speed to '{0}'.";
        public string SetWaveSpeedNoParse = "Couldn't parse wave speed '{0}'.";

        public string HasWater = "This planet already has water";
        public string CreateWater = "Created water at your closest planet. Use /wradius to set up the water's radius. /wrradius will tell you the radius at your position";
        public string RemoveWater = "Removed the water at your closest planet.";

        public string ExportWater = "Exported water settings to clipboard.";
        public string ImportWater = "Imported water settings.";

        public string Depth = "Depth: {0}m";
        public string Altitude = "Altitude: {0}m";
        public string ToggleShowDepth = "Toggled rendering of depth.";
        public string ToggleShowAltitude = "Toggled rendering of altitude.";

        public string ToggleBirds = "Toggled birds at your closest planet.";
        public string ToggleFish = "Toggled fish at your closest planet.";

        public string ToggleFog = "Toggled fog.";

        public string GenericNoParse = "Could not parse '{0}'.";
        public string GenericNoPermissions = "You do not have permissions to run this command.";
        public string GenericNoCreative = "You must be in creative mode to toggle fog.";
        public string GenericMinArgs = "Not enough args. '{0}' Minimum";
        public string GenericMaxArgs = "Too many args. '{0}' Maximum";
        public string GenericNotEnoughArgs = "Not enough args";

        public string ToggleDebug = "Toggled debug mode.";

        public string GetVolume = "Your water volume '{0}'.";
        public string SetVolume = "Set water volume to '{0}'.";
        public string SetVolumeNoParse = "Couldn't parse water volume '{0}'.";

        public string GetWaveScale = "The water's wave scale is '{0}'.";
        public string SetWaveScale = "Set the water's wave scale to '{0}'.";
        public string SetWaveScaleNoParse = "Couldn't parse wave scale '{0}'.";

        public string GetTexture = "The water's texture is '{0}'.";
        public string SetTexture = "Set the water's texture to '{0}'.";
        public string SetTextureNoFind = "Couldn't find texture '{0}'.";

        public string GetMaterial = "The fluid's material is '{0}'.";
        public string SetMaterial = "Set the fluid's material to '{0}'.";
        public string SetMaterialNoFind = "Couldn't find material '{0}'.";

        public string ListTextures = "Available textures: ";
        public string NoTextures = "Couldn't find any textures.";

        public string ListMaterials = "Available materials: ";
        public string NoMaterials = "Couldn't find any materials.";

        public string ListCommands = "Available commands: ";

        public string NoTextAPI = "You do not have Text HUD API installed.";

        public string GetCrushDamage = "The water's crush damage is '{0}'.";
        public string SetCrushDamage = "Set the water's crush damage to '{0}'.";
        public string SetCrushDamageNoParse = "Couldn't parse crush damage '{0}'.";

        public string TogglePlayerDrag = "Toggled player drag.";

        public string ToggleTransparency = "Toggled transparency.";

        public string ToggleLighting = "Toggled lighting.";

        public string GetCollectRate = "The water's collection rate is '{0}'.";
        public string SetCollectRate = "Set the water's collection rate to '{0}'.";
        public string SetCollectRateNoParse = "Couldn't parse collection rate '{0}'.";

        public string GetFogColor = "The water's fog color is '{0}'.";
        public string SetFogColor = "Set the water's fog color to '{0}'.";
        public string SetFogColorNoParse = "Couldn't parse fog color '{0}'.";

        public string GetTideHeight = "The water's tide height is '{0}m'.";
        public string SetTideHeight = "Set the water's tide height to '{0}m'.";
        public string SetTideHeightNoParse = "Couldn't parse tide height '{0}'.";

        public string GetTideSpeed = "The water's tide speed is '{0}'.";
        public string SetTideSpeed = "Set the water's tide speed to '{0}'.";
        public string SetTideSpeedNoParse = "Couldn't parse tide speed '{0}'.";

        public string GetWaterSettings = "The water's settings are:";

        public string ToggleFoam = "Toggled surface foam.";

        public string GetDensity = "The fluid's density is '{0}' kg/m³.";
        public string SetDensity = "Set the fluid's density to '{0}' kg/m³.";
        public string SetDensityNoParse = "Couldn't parse fluid density '{0}'.";

        public string OpenGuide = "Opened the steam guide through Steam Overlay.";
        public string OpenDiscord = "Opened discord through Steam Overlay.";

        public string GetRelativeRadius = "The radius relative to your camera is '{0}'";

        public string UIInputNumber = "Enter number value [{0}]";

        public string UIRefreshData = "Refresh";

        public string UIShowFog = "Show Fog [{0}]";
        public string UIWaterSettings = "Closest Water";
        public string UIWaterWaveSettings = "Wave";
        public string UIWaterTideSettings = "Tide";
        public string UIWaterVisualSettings = "Visual";
        public string UIWaterGameplaySettings = "Gameplay";
        public string UIWaterCurrentSettings = "Currents";

        public string UIWaterBuoyancy = "Buoyancy Multiplier [{0}]";
        public string UIWaterCollectionRate = "Resource Collection Rate [{0}]";
        public string UIWaterCrushDamage = "Crush Damage [{0}]";
        public string UIWaterEnableFish = "Enable Fish [{0}]";
        public string UIWaterEnableFoam = "Enable Sea Foam [{0}]";
        public string UIWaterEnableSeagulls = "Enable Birds [{0}]";
        public string UIWaterFogColor = "Fog Color";
        public string UIWaterLit = "Enable Lighting [{0}]";
        public string UIWaterMaterialId = "Physics Material [{0}]";
        public string UIWaterPlayerDrag = "Enable Player Drag [{0}]";
        public string UIWaterRadius = "Water Radius [{0}]";
        public string UIWaterTexture = "Surface Texture [{0}]";
        public string UIWaterTideHeight = "Tide Height [{0}]";
        public string UIWaterTideSpeed = "Tide Speed [{0}]";
        public string UIWaterTransparent = "Transparent [{0}]";
        public string UIWaterWaveHeight = "Wave Height [{0}]";
        public string UIWaterWaveScale = "Wave Scale [{0}]";
        public string UIWaterWaveSpeed = "Wave Speed [{0}]";
        public string UIWaterCurrentScale = "Current Scale [{0}]";
        public string UIWaterCurrentSpeed = "Current Speed [{0}]";

        public string UILanguage = "Language [{0}]";
        public string UIQuality = "Quality [{0}]";
        public string UIVolume = "Volume [{0}]";
        public string UIShowCenterOfBuoyancy = "Show Center of Buoyancy [{0}]";
        public string UIShowDebug = "Show Debug [{0}]";
        public string UIShowAltitude = "Show Altitude [{0}]";
        public string UIShowDepth = "Show Depth [{0}]";
    }
}
